...
# originally: bush,snares,recent,pred
# rs=subset(r,c("snares","recent","pred"))
# predss=subset(preds,c("bush","interface","dist.roads","dist.i"))
# r=addLayer(rs,predss)
...
# sort m ==============================================
p=subset(preds,c("dem","bush","interface","dist.roads","dist.i"))
px=setDT(extract(p,bl,df=T,cellnumbers=T))
mz=cbind(m,px[,.(dem,interface,dist.roads,dist.i)])
setorder(mz,dist.roads,dist.i)
...
usethis::use_data(mz,overwrite=T)
